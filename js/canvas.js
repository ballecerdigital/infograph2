...
function renderElement(element, selectedId) {
  ...
  node.className = `info-node ${isSelected ? 'selected' : ''} align-${element.align || 'left'} variant-${element.variant || 'glass'}`;
  ...
  node.style.setProperty('--photo-radius', `${element.radius ?? 24}px`);
  node.style.setProperty('--photo-pos-x', `${element.photoX ?? 50}%`);
  node.style.setProperty('--photo-pos-y', `${element.photoY ?? 50}%`);
  node.style.setProperty('--photo-shadow', `${element.shadow ?? 1}`);
  node.style.fontSize = `${element.fontSize || 22}px`;
  node.appendChild(renderContent(element));
  if (isSelected) node.appendChild(renderResizeHandles(element));
  return node;
}
...
function renderResizeHandles(element) {
  ...
  const gripNames = ['nw', 'n', 'ne', 'e', 'se', 's', 'sw', 'w'];
  handles.innerHTML = gripNames.map((name) => `<span class="resize-handle handle-${name}" data-resize-handle="${name}"></span>`).join('');
  if (element.type === 'photo') {
    const badge = document.createElement('div');
    badge.className = 'resize-badge';
    badge.innerHTML = '<span class="material-symbols-outlined">open_with</span> Drag • Resize';
...
function renderContent(element) {
  ...
  if (element.type === 'photo') {
    ...
    const shadowClass = element.shadow === 0 ? 'no-shadow' : element.shadow >= 2 ? 'strong-shadow' : 'soft-shadow';
    wrap.innerHTML = `<div class="photo-card photo-fit-${fit} ${shadowClass}">${element.src ? `<img src="${element.src}" alt="Uploaded visual">` : '<div class="photo-placeholder"><span class="material-symbols-outlined">add_photo_alternate</span></div>'}</div>`;
    return wrap;
  }
...
function onPointerDown(event) {
  ...
  activeDrag = {
    ...
    handle: resizeHandle?.dataset.resizeHandle || null,
...
function onPointerMove(event) {
  ...
  if (activeDrag.mode === 'resize') {
    const patch = calculateResize(activeDrag, dx, dy, element);
    updateElement(activeDrag.id, patch, false);
    requestAnimationFrame(updateSelectionToolbar);
    return;
  }
...
function calculateResize(start, dx, dy, element) {
  const handle = start.handle || 'se';
  const size = getCanvasSize();
  const minW = element.type === 'divider' ? 64 : element.type === 'photo' ? 96 : 48;
  const minH = element.type === 'divider' ? 6 : element.type === 'photo' ? 80 : 28;
  const snap = currentState.snapToGrid ? 6 : 1;
  let x = start.x;
  let y = start.y;
  let w = start.w;
  ...
  // Keep resize usable while still allowing a little creative bleed.
  x = clampValue(x, -w + 30, size.w - 30);
  y = clampValue(y, -h + 30, size.h - 30);
  w = clampValue(w, minW, size.w * 1.4);
  h = clampValue(h, minH, size.h * 1.4);

  updateDragGuides(null, null, (currentState.zoom || 42) / 100);
  return { x, y, w, h };
}
