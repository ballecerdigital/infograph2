...
function renderContentFields(element) {
  ...
  if (element.type === 'photo') {
    ...
    controls.className = 'stack';
    controls.innerHTML = `
      <div class="row"><button class="button" id="changePhotoBtn"><span class="material-symbols-outlined">add_photo_alternate</span>Choose photo</button><button class="button ghost" id="clearPhotoBtn">Clear</button></div>
      <div class="segmented" id="photoFitControls">
        <button class="segment-button ${element.fit !== 'contain' ? 'active' : ''}" data-fit="cover">Fill</button>
        <button class="segment-button ${element.fit === 'contain' ? 'active' : ''}" data-fit="contain">Fit</button>
      </div>
      <label class="field">Photo radius<input id="photoRadiusRange" type="range" min="0" max="64" value="${element.radius ?? 24}"></label>
      <label class="field">Focus X<input id="photoXRange" type="range" min="0" max="100" value="${element.photoX ?? 50}"></label>
      <label class="field">Focus Y<input id="photoYRange" type="range" min="0" max="100" value="${element.photoY ?? 50}"></label>
      <label class="field">Shadow<select id="photoShadowSelect"><option value="0">None</option><option value="1">Soft</option><option value="2">Strong</option></select></label>`;
    ...
    controls.querySelector('#photoFitControls').addEventListener('click', (event) => {
      const button = event.target.closest('[data-fit]');
      if (!button) return;
      updateElement(element.id, { fit: button.dataset.fit }, true);
    });
    controls.querySelector('#photoRadiusRange').addEventListener('input', (event) => updateElement(element.id, { radius: Number(event.target.value) }, false));
    controls.querySelector('#photoXRange').addEventListener('input', (event) => updateElement(element.id, { photoX: Number(event.target.value) }, false));
    controls.querySelector('#photoYRange').addEventListener('input', (event) => updateElement(element.id, { photoY: Number(event.target.value) }, false));
    controls.querySelector('#photoShadowSelect').value = String(element.shadow ?? 1);
    controls.querySelector('#photoShadowSelect').addEventListener('change', (event) => updateElement(element.id, { shadow: Number(event.target.value) }, true));
    box.appendChild(controls);
  }
